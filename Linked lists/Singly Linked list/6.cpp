...
void insert_at_particular_index(node *p, int x, int pos)
{
    ...
    point = head;
    for (int i = 0; i < pos - 1; i++) //to traverse the point pointer to pos-1 index
    {
        point = point->next; //for moving pointer to next location
    }
    t->next = point->next; //assinging popinter next value to t next value
    point->next = t;       //assining t adddress to pointer next
}
